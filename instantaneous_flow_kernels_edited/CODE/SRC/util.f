! *dk readobs
      subroutine readobs(ofield1,ofield2,ofield3)
      implicit none
      ...
      real ofield1(grid, grid2)
      real ofield2(grid, grid2)
      real ofield3(grid, grid2)
      real lon(grid, grid2)
      real lat(grid, grid2)
      ...
      open(195,file=trim(reference)//'DynTop', action = 'read',
     &  status = 'unknown')
      open(196,file=trim(reference)//'Ge-oid', action = 'read',
     &  status = 'unknown')
      open(197,file=trim(reference)//'CMBTOP', action = 'read',
     &  status = 'unknown')
      do i=1,grid
         do j=1,grid2
	  read(195,*) lon(i,j), lat(i,j), ofield1(i,j)
	  read(196,*) lon(i,j), lat(i,j), ofield2(i,j)
	  read(197,*) lon(i,j), lat(i,j), ofield3(i,j)
         enddo
      enddo
      close(195)
      close(196)

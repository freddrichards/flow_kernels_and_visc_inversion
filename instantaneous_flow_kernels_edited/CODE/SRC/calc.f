      subroutine fldcalc(GeoK,VelK,SurK,CMBK,mfcalc,oSurTop,ogeoid,
     &  oCMBtop)
      implicit none
      ...
      real oSurTop(grid, grid2)
      real ogeoid(grid, grid2)
      real oCMBTop(grid, grid2)
      real m1(grid, grid2)
      real m2(grid, grid2)
      real m3(grid, grid2)
...
!                     GeoFld(i,j)=GeoFld(i,j)+RefGeo  (l,m)*sphharm
		     m1(i,j)=sqrt((SurTOP(i,j)-oSurTOP(i,j))**2.)
		     m2(i,j)=sqrt((geoid(i,j)-ogeoid(i,j))**2.)
		     m3(i,j)=sqrt((CMBTop(i,j)-oCMBTop(i,j))**2.)
...
!	  Calculate misfit between observed and predicted fields 
      mfcalc=(((w1*sum(m1))/(grid*grid2))+((w2*sum(m2))/(grid*grid2))
     &  +((w3*sum(m3))/(grid*grid2)))*(1./(w1+w2+w3))

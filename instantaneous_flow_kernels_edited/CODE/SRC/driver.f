      ...
      common /path/ output, input, name_prof, reference
      
      integer N, sufi, suff, step
      character*120 output, input, name_prof, reference
      real :: start, finish
...
!       call cpu_time(start)
      ...
      if (N .ne. 4) then
         write(*,'(A)') 'Number of given arguments should be 4!'
         write(*,'(I1.1, A)') N, ' is given. Try the following order:'
         write(*,'(A)')
     &        './KERNELS [INPUT] [OUTPUT] [VISC_PROFILE] [REFERENCE]'
         stop
      endif
      ...
      call getarg(4,reference)   ! char1 is sufi
...
! --------------------------------------------------
      ...
      real :: start, finish
      character*80 output, input, name_prof, reference
...
!      write(*,*) 'done kernel build' 
!       write(*,'(A)') '-------------------------------------'
!       write(*,'(/, 5X, A, /)') 'Calculation of The Fields!'
!       write(*,'(A)') '-------------------------------------'
 
      call fldcalc(kernel,velkernel,topsurfkernel,topcmbkernel,mf,
     &	oDT,oG,oCMB)
!      write(*,*) 'done field calculation' 
!      write(*,'(A)') '-------------------------------------'
       write(*,'(f15.8)') mf
!      write(*,'(A)') '-------------------------------------'
!       if (CorrFlag == 1) then 
...
!       else
!       write(*,'(A)') '-------------------------------------'
!       write(*,'(/, 5X, A, /)') 'Computation Successful!'
!       write(*,'(A)') '-------------------------------------'
!       endif
!       open(194,file=trim(output)//'Correl.dat',action='write',
!      & status='unknown')
!       write(194,*) Corr, geomax,surmax
!       close(194)
      
      !need to edit summary (in util.f) so just prints misfit
!      call summary
